...
//    }
	...
	@GetMapping({"/home"})
    public String getHomePage(Model model, @ModelAttribute("user") Users user, HttpSession session) {
		if(user.getRole() == null) {
			return "login/loginPage";
		}
		else if(user.getRole().equalsIgnoreCase("1")) {
			return "redirect:/admin";
		}
		else {
			Integer diligenceId = customSession.diligenceId();
